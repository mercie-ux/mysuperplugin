...
window.app = Vue.createApp({
  ...
  methods: {
    mysuperpluginFunction(data) {
      LNbits.api
        .request(
          'GET', // Type of request
          '/mysuperplugin/api/v1/test/00000000', // URL of the endpoint
          this.g.user.wallets[0].inkey, // Often endpoints require a key
          data
        )
        .then(response => {
          this.mysuperpluginData.push(mapObject(response.data)) // Often whats returned is mapped onto some model
        })
    ...
    getVettedReadme() {
      ...
      LNbits.api
        .request('GET', '/mysuperplugin/api/v1/vetted', this.g.user.wallets[0].inkey)
        .then(response => {
  ...
  // To run on startup
  created() {
    this.mysuperpluginFunction('lorum')
    this.initWs()
